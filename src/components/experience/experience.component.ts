        ...
        <div class="relative border-l-2 border-slate-800 ml-3 md:ml-6 space-y-12">
          @for (job of jobs; track job.company) {
            <div class="relative pl-8 md:pl-12">
              ...
              <div class="bg-slate-900/80 p-6 md:p-8 rounded-xl border border-slate-800 hover:border-slate-700 transition-all">
                <div class="flex flex-col md:flex-row md:justify-between md:items-start mb-4 gap-2">
                  <div>
                    <h3 class="text-xl md:text-2xl font-bold text-white">{{ job.role }}</h3>
                    <div class="text-blue-400 font-medium text-lg">{{ job.company }}</div>
                  </div>
                  <div class="flex flex-col items-start md:items-end">
                    <span class="text-slate-400 font-mono text-sm bg-slate-800 px-3 py-1 rounded-full border border-slate-700">
                      {{ job.period }}
                    </span>
                    <span class="text-slate-500 text-sm mt-1">{{ job.location }}</span>
                  </div>
                </div>
                
                <ul class="space-y-3">
                  @for (point of job.description; track point) {
                    <li class="flex items-start text-slate-400 leading-relaxed">
